...
const CrmCash3Full = ({setErr}) => {
    ...
    const handleThingClick = (thing, typeThing) => {
        ...
        // } else {
        //     newThisOrderToSend.orderunits = [...selectedThings2, {...thing, amount: 1, orderunitunits: [], x:thing.x, y:thing.y}]
        // }
        let dataToSend = {
        ...
        axios.post(`/api/order/addThing`, dataToSend)
            .then(response => {
                    ...
                    <div className="" style={{width: '53.5vw'}}>
                        {products === null ? (
                            ...
                            <div className="" style={{border: "solid 1px #cccabf",}}>
                                        ...
                                        <Row xs={1} md={5} className="">
                                            {products.rows.map((thing, index) => (
                                                <Col key={index} onClick={() => handleThingClick(thing, "prodU")}
                                                     ...
                                                     style={{overflow: 'hidden', whiteSpace: "nowrap", textOverflow: "ellipsis", border: "solid 2px #cccabf"}}
                                                >
                                                    {thing.name}
                                                </Col>
                                            ))}
                                ...
                                <div className="d-flex align-items-center align-content-center m-1 p-1" style={{overflow: 'auto', border: "solid 1px #cccabf", width: "99%"}}>
                                    {uniqueTypesProduct.map((thing, index) => (
                                        <div key={index} onClick={(event) => setTypeSelect2(thing.type)}
                                             className="adminFont btn text-center hoverBlack"
                                             style={{whiteSpace: "nowrap", textOverflow: "ellipsis", border: "solid 1px #cccabf"}}
                                        >
                                            {thing.type}
                                        </div>
                                    ))}
